...
void SaveCurrentData(char* name, Triangle* t, Rectangle* r, int CT, int Mode){
	FILE* fp;
	if((fp = fopen(name, "w")) == NULL) exit(0);
	fprintf(fp, "no.1{");
	for(int i = 0; i < 5; i++){
		fprintf(fp, "%c{%lf,%lf,%lf,%lf,%lf,%lf}\n", 
			'A'+i, 
			((t+i)->A[0]).x, ((t+i)->A[0]).y, 
			((t+i)->ab[0]).x, ((t+i)->ab[0]).y, 
			((t+i)->ab[2]).x, ((t+i)->ab[2]).y);
	}
	for(int i = 0; i < 2; i++){
		fprintf(fp, "%c{%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf}\n",
			'F'+i,
			((r+i)->A[0]).x, ((r+i)->A[0]).y, 
			((r+i)->ab[0]).x, ((r+i)->ab[0]).y, 
			((r+i)->ab[1]).x, ((r+i)->ab[1]).y, 
			((r+i)->ab[2]).x, ((r+i)->ab[2]).y);
	} 
	fprintf(fp, "}\n");
	fprintf(fp, "(%d,%d)", CT, Mode);
	if(fclose(fp)) exit(0);
}

void ReadSavedData(char* name, Triangle* t, Rectangle* r, int* CT, int* Mode){
	for(int i = 0; i < 5; i++){
		GetTriPosition(name, 1, 'A'+i, t+i);
	}
	for(int i = 0; i < 2; i++){
		GetRecPosition(name, 1, 'F'+i, r+i);
	}
	FILE* fp;
	if((fp = fopen(name, "r")) == NULL) exit(0);
	while(fgetc(fp) != '(');
	fscanf(fp, "%d,%d", CT, Mode);
	if(fclose(fp)) exit(0);
}
