...
int PointCloseLine(Point* p, Point* p1, Point* p2){
	Point a = PointMinus(p, p1);
	Point b = PointMinus(p, p2);
	if(pointProduct(&a, &b) < 0.05) return 1;
	return 0;
} 
...
//					pt->attached = 1; 
	...
	ans = TriAttachOneLine(pt, Outlines+cnt-1, Outlines);
	if(ans.x || ans.y){
//				pt->attached = 1; 
...
//						pt->attached = 1; 
	...
	for(i = 0; i < 2; i++){
		...
		for(j = 0; j < 4; j++){
			ans = TriAttachOneLine(pt, &(r[i].A[j%4]), &(r[i].A[(j+1)%4]));
			if(ans.x || ans.y){
//						pt->attached = 1; 
...
//			pr->attached = 1;
	...
	ans = RecAttachOneLine(pr, Outlines+cnt-1, Outlines);
	if(ans.x || ans.y){
//		pr->attached = 1;
		return ans;
	}
...
//				pr->attached = 1;
	...
	for(i = 0; i < 2; i++){
		if(!((r+i)->attached)) continue;
		for(j = 0; j < 4; j++){
			ans = RecAttachOneLine(pr, &(r[i].A[j%4]), &(r[i].A[(j+1)%4]));
			if(ans.x || ans.y){
//				pr->attached = 1;
				return ans;
			}
